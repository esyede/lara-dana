...
class CreateOrder
{
    ...
    public function payload(): array
    {
        ...
        return array_merge($mandatories, $this->shopInfo(), $this->mcc(), $this->paymentPreference());
    }
    ...
    {
        $goods = collect(Arr::get($this->order, 'goods', []))->map(function ($item) {
            return [
                'merchantGoodsId' => Arr::get($item, 'merchantGoodsId', ''),
                'description' => Arr::get($item, 'description', ''),
                'category' => Arr::get($item, 'category', ''),
                'price' => [
                    'currency' => Arr::get($item, 'price.currency', $this->currency),
                    'value' => Arr::get($item, 'price.value', 0)
                ],
                'unit' => Arr::get($item, 'unit', ''),
                'quantity' => Arr::get($item, 'quantity', ''),
                'snapshotUrl' => Arr::get($item, 'snapshotUrl', '[]'),
                'merchantShippingId' => Arr::get($item, 'merchantShippingId', ''),
                'extendInfo' => Arr::get($item, 'extendInfo', ''),
            ];
        })->toArray();
        ...
        $orderData = [
            ...
            'orderAmount' => [
                ...
                // Dana uses 'Cent' by default, 100 cents = Rp 1, see:  https://dashboard.dana.id/api-docs/read/31#Money
            ...
            'createdTime' => Arr::get($this->order, 'createdTime', now()->format(config('dana.date_format'))),
            'expiryTime' => Arr::get($this->order, 'expiryTime', now()->addMinutes(config('dana.expired_after', 60))->format(config('dana.date_format'))),
            'goods' => $goods,
    ...
    public function notificationUrls(): array
    {
        return Arr::get($this->body, 'notificationUrls', [
            ['url' => config('dana.pay_return_url'), 'type' => 'PAY_RETURN'],
            ['url' => config('dana.order_notify_url'), 'type' => 'NOTIFICATION']
        ]);
    }
    ...
    public function mcc(): array
    {
        if (Arr::get($this->body, 'mcc', null)) {
            return ['mcc' => Arr::get($this->body, 'mcc', '')];
        }
    ...
    public function paymentPreference()
    {
        if (collect($this->paymentPreference)->isNotEmpty()) {
            $payOptionBills = collect(Arr::get($this->paymentPreference, 'payOptionBills', []))->map(
                function ($payOption) {
                    return [
